...
socket.on("connect", () => {  
  console.log("connected: +"+ socket.id); // "G5p5..."
});
// socket.emit('add-user', "this.userName)";
socket.on('movement', function (data) {
  console.log("got data")
  if(data.id != id){
    animateVRM(currentVrm, data.results);
  }
});
...
// scene.background = new THREE.Color("#f1c40f");
// light
...
// setInterval(()=> { 
//   let {faceLandmarks, multiFAceGreometry, poseLandmarks} = tmp
//   socket.emit('movement', {id:0,results: JSONencode({faceLandmarks, multiFAceGreometry, poseLandmarks})})
//   console.log("emitting",{id:0,resuts: tmp} )
...
setTimeout(()=> socket.emit('movement', { id: 0, results: {}}, 30000));
...
camera.start();
function sendmov(){
  socket.emit('movement', { id: 0, results: {}})
  console.log("button clicked")
}
document.querySelector('button').addEventListener('click', sendmov)